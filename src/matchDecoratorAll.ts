// Adapted from https://github.com/codemirror/view/blob/main/src/matchdecorator.ts

// Modified to search for matches in the entire document (not just those in the viewport's visible ranges)
// in order to support arrows that extend beyond the viewport in long documents
...
function matchRanges(view: EditorView, maxLength: number) {
  ...
  let visible = [{from: 0, to: view.state.doc.length}];
...
export class MatchDecoratorAll {
  ...
  private updateRange(view: EditorView, deco: DecorationSet, updateFrom: number, updateTo: number) {
    // for (let r of view.visibleRanges) {
      const r = {from: 0, to: view.state.doc.length};
